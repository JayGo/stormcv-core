 ...
 *
 */
public class XuggleStreamReader extends MediaListenerAdapter implements Runnable {
	...
	public XuggleStreamReader(String streamId, String streamLocation, String imageType, int frameSkip, int groupSize, int sleepTime, LinkedBlockingQueue<Frame> frameQueue) {
	...
	private void processFrame(BufferedImage image) {
		...
		if (frameNr % frameSkip < groupSize) {
			try {
				buffer = ImageUtils.imageToBytes(image, imageType);
			} catch (IOException e) {
				e.printStackTrace();
			}
