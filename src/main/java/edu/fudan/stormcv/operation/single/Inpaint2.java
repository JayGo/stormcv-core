...
import edu.fudan.stormcv.operation.GradientCalculator;
...
//		Inpaint2 inpainter = new Inpaint2();
//		
//		BufferedImage image;
//		try {
...
//	}
    ...
    public BufferedImage init(BufferedImage a_origImg, BufferedImage a_fillImg, Boolean quickInpaint) {
        halt = false;
        int i, j;
        countInpiant = 0;
        try {
        ...
        while (flag) {
			...
			if (best[0] == 0 && best[1] == 0 && best[2] == 0 && best[3] == 0) {
				best=bestExemplar(Hp, toFillTrans, initialSourceRegion,pixelmap,best1, ih,0);
			}*/
...
//		} 
    ...
    int[] bestExemplar2(int[][] Hp, double[][] toFill, int[][] sourceRegion, Boolean quickInpaint) {
        int[][] Ip = new int[toFill.length][toFill[0].length];
        for (int i = 0; i < toFill[0].length; i++) {
            for (int j = 0; j < toFill.length; j++) {
                int col = Hp[i][j] / ih;
